...
let ProcessCommandLine (token: CancellationToken) sendCode (argv: string []) =
    task {
            ...
            [|
                for arg in argv do
                    ...
                    //useEditFiles <- true
                        ...
                        printfn ""
                        printfn
                            "*** NOTE: if sending the code to a device the versions of CodeModel.fs and Interpreter.fs on the device must match ***"
                        printfn ""
