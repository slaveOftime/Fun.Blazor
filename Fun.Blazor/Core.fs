...
type IStoreManager =
    ...
    /// For example, with this we can abstract the FSharp.Control.Reactive out to a separate pacakges to consume to help reduce the WASM size. If uses do not want to use it.
    abstract GetOrAddDisposableStore : string * (unit -> IDisposable) -> IDisposable

    /// This is for library authors. Can be used to add subscriptions resources, so we can clean it up when the store manager is disposing.
    abstract AddDispose : IDisposable -> unit
