...
let ``ref should work for component`` () =
    ...
    let demo = MudButton'() { ref (fun _ -> i <- 2) }
...
let ``renderMode should work for html blazor`` () =
    ...
    let demo =
        html.blazor<CascadingValue<int>> (RenderMode.InteractiveServer, attr = (nameof (Unchecked.defaultof<CascadingValue<int>>.Value) => 123))
