...
module ServerTests =
    let createTestHost (configureServices: IServiceCollection -> unit) (configureApp: IApplicationBuilder -> unit) =
        let host =
            Host
                .CreateDefaultBuilder()
                .ConfigureWebHost(fun webHost ->
                    webHost.UseTestServer().UseSetting("test", "123").ConfigureServices(configureServices).Configure(configureApp)
                    |> ignore
                )
                .Build()
    ...
    let makeTestHost (setRoute: IEndpointRouteBuilder -> unit) =
        createTestHost
            (fun services ->
                services.AddRouting() |> ignore

                services.AddServerSideBlazor(fun options -> options.RootComponents.RegisterCustomElementForFunBlazor<ServerDemoCounter>())
                |> ignore

                services.AddRazorComponents() |> ignore
                services.AddFunBlazorServer() |> ignore
            )
            (fun builder -> builder.UseRouting().UseEndpoints(setRoute) |> ignore)
    ...
    [<Fact>]
    let ``MapGet should work with DI`` () =
        task {
            use server =
                makeTestHost (fun route ->
                    route
                        .MapGet(
                            "/test",
                            Func<_, _>(fun (config: IConfiguration) ->
                                task {
                                    do! Task.Delay 100
    ...
    [<Fact>]
    let ``MapRazorComponentsForSSR and MapCustomElementsForSSR should work`` () =
        task {
    ...
    [<Fact>]
    let ``MapCustomElementsForSSR should not work for wrong convention`` () =
        task {
            let notFoundNode = div { "not found" }
    ...
    [<Fact>]
    let ``scoped style sheets should work`` () =
        task {
            use server =
                makeTestHost (fun route ->
                    let route = route.MapGroup("").AddFunBlazor()
    ...
    [<Fact>]
    let ``Parsable parameters should work`` () =
        task {
            use server =
                makeTestHost (fun route ->
                    route.MapRazorComponents() |> ignore
            ...
            Assert.StartsWith("""<div><div>Some({| Age = 30 |})</div><div>[7; 8]</div><div>[{| Age = 34 |}]</div></div>""", actual)
        }
